...
static uint8_t new_thread0_stack[2048] BSP_PLACE_IN_SECTION(".stack.new_thread0") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
...
void new_thread0_create(void)
{
    ...
    UINT err;
    err = tx_thread_create (&new_thread0, (CHAR*) "New Thread", new_thread0_func, (ULONG) 0, &new_thread0_stack, 2048,
                            3, 3, 1, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
